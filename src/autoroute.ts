    ...
    return {
      ...
      message: {
        ...
        content: useCompactBoundedReviewPrompt ? [resumeContext, compactBoundedReadOnlyReviewSteeringMessage(promptText, ctx.hasUI)].filter(Boolean).join("\n\n") : useCompactPathPrompt ? [resumeContext, compactPathSteeringMessage(promptText, ctx.hasUI)].filter(Boolean).join("\n\n") : useCompactGeneralPrompt ? [resumeContext, compactGeneralSteeringMessage(promptText)].filter(Boolean).join("\n\n") : [
          ...
          "When routing, choose topology deliberately: single scout for branch/project structure; chain scout -> planner -> reviewer for architecture/migration strategy; chain scout -> reviewer for project-wide review/test/tooling/command/policy audits; chain scout -> planner -> worker -> reviewer for risky implementation or surgical long-file edits; parallel planners/reviewers for independent option comparison; DAG scout/planner -> parallel non-overlapping workers -> reviewer for independent implementation slices; DAG scout -> parallel folder/context agents -> reviewer for deep folder fan-out. Do not route plain memory recall/inventory; use chalin_memory_search. Planner is required for risky implementation and surgical long-file chains; scout -> worker -> reviewer is incomplete.",
