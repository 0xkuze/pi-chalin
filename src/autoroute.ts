...
import { buildChalinOrchestratorSystemPrompt } from "./orchestration.ts";
import { isUsableStepHandoff, loadResumableRunState } from "./runner-state.ts";
import { beginChalinTurn, getDirectChangedPaths, getDirectCriticalGuardContextMessage, isSemanticPolicyJudgeRequestFresh, recordDirectToolCompletion, recordDirectToolStart, recordSemanticPolicyJudgeResult } from "./runtime-state.ts";
import { formatSemanticPolicyJudgeSteer, runSemanticPolicyJudge, shouldApplySemanticPolicyJudgeResult } from "./semantic-policy-judge.ts";
  ...
  pi.on("before_agent_start", (event, ctx) => runHookEffect("autoroute.beforeAgentStart", async () => {
    ...
    return {
      ...
      message: {
        ...
        content: [resumeContext, "Use the system orchestration contract. Decide DIRECT or ROUTE; if ROUTE, call one `chalin_route` with topology `sequential` or `dag`."].filter((line): line is string => Boolean(line)).join("\n\n"),
        display: false,
      },
    };
  }));
