...
export function buildChalinOrchestratorSystemPrompt(agents: AgentDefinition[]): string {
  ...
  return [
    ...
    "- Risky long-file/surgical edit is the explicit no-rewrite case: use worker -> reviewer by default; add planner only when a separate target-region plan materially reduces rewrite risk.",
