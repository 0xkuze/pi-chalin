...
export const ORCHESTRATION_EVAL_CASES: MeshOrchestrationEvalCase[] = [
  ...
  {
    id: "single-command",
    prompt: "corre npm test y dime si pasa",
    expectedDecision: "direct",
    expectedTopology: "none",
    expectedAgents: [],
...
export function buildCompactMeshOrchestratorSystemPrompt(): string {
  return [
    ...
    "Direct bounded code work: do not narrate planning or emit visible analysis before tools; first inspect briefly, then write the requested implementation/test/docs promptly, run the nearest relevant verification command, fix failures, rerun verification after the final edit, and answer immediately with changed paths, verification result, and one note about the satisfied constraint.",
