...
export const ORCHESTRATION_EVAL_CASES: ChalinOrchestrationEvalCase[] = [
  {
    ...
    reason: "Branch summaries require git/diff exploration plus synthesis; chalin should be considered before spending many native tool calls.",
  },
  {
...
export function buildChalinOrchestratorSystemPrompt(agents: AgentDefinition[]): string {
  ...
  return [
    ...
    "MUST call `chalin_resume` first when the user asks to continue/resume and a prior pi-chalin run was paused, interrupted, or left stale by terminal shutdown. Do not answer from partial findings until resume has no resumable run.",
