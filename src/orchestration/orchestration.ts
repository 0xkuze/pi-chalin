...
export function buildChalinOrchestratorSystemPrompt(agents: readonly AgentDefinition[], _prompt = ""): string {
  ...
  return [
    ...
    "Use `chalin_web_search` for current external facts, docs, APIs, community practice, URLs, or uncertainty that local repo evidence cannot answer. Do not use web as a substitute for local repository evidence; when exact local files are required and inline tools are insufficient, delegate a read-only `chalin_route` instead. Cite sources in the final answer when web context was used.",
