...
export function buildChalinOrchestratorSystemPrompt(agents: readonly AgentDefinition[]): string {
  ...
  return [
    ...
    "Use `chalin_bash_job` inline for potentially long/blocking verification, build, typecheck, test-suite, dev-server, watcher, or sleep/server commands when Primary Pi can otherwise keep the task in one compact loop. Set requiredEvidence=true when that result is needed before claiming completion. Prefer start + independent work; set completionAction=resume when Pi should wake after the job finishes. Do not call await immediately unless the result is needed for the very next step.",
