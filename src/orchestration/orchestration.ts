...
export function buildChalinOrchestratorSystemPrompt(agents: readonly AgentDefinition[]): string {
  ...
  return [
    ...
    "Use normal `bash` by default for shell commands, especially quick checks or commands whose output is needed before the next assistant response. Use `chalin_bash_job` only when you would otherwise reach for a long timeout, a dev-server/watcher/sleep/server command, a likely slow full test/build/typecheck/CI verification, or a command that can run while you continue independent work. Do not start a background job if the next thing you will do is simply wait for it; use normal `bash` with an appropriate timeout instead. Set requiredEvidence=true when that result is needed before claiming completion, and set completionAction=resume when Pi should wake after the job finishes.",
