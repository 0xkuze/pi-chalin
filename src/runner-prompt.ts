...
export function buildSdkPrompt(
  ...
  return [
    ...
    "- Native read/find/grep/ls inspect; edit/write change files. Prefer native diffs over shell when clearer.",
    ...
    mutationRelevant ? "- Impl/test: derive the contract from prompt+repo evidence. Tests are contract oracles: preserve starter assertions unless disproven; add focused criteria plus one boundary/counterexample; cover changed and preservation/no-op/composition paths. Preserve public compatibility: no stricter errors, normalization, mutation, or API-shape changes unless evidence requires. Narrow token/flag/path/format means change only that part and add adjacent preservation; not family rewrites. Delimited/protected segment separate even when adjacent -> assert prev+segment+next separation. Invalid/reject requirements are contract. Validate config/resource/range/window invariants: empty input, invalid types/bounds, relational bounds; split cheap validation tests. Capture normalized config so caller-side object mutation cannot alter runtime semantics. Time/window/retry/cache/rate/budget behavior prefers internal test seams or runner-native fake timers without expanding public APIs; avoid global monkeypatches and wall-clock sleeps. Text/query filters: trim/blank, no-match, order." : undefined,
    mutationRelevant ? "- Code behavior changes update nearest tests unless existing assertions cover every criterion; final distinguishes edited tests from evidence-only tests. Test files register runner-discoverable cases; zero-test assertion scripts are invalid." : undefined,
    mutationRelevant ? "- Scaffold/package work: requested files, metadata, entrypoints, build output, tests, docs, and language/toolchain agree. TypeScript requests use TypeScript tests unless repo convention proves otherwise; do not swap to JS for simplicity. Prefer native runners unless requested/established; tests use the runner's discoverable API. CLI bins target real executables; CLI tests cover API + real command path, with args/no-input when relevant." : undefined,
    mutationRelevant ? "- Parser/scanner/state-machine/normalization changes follow repo grammar evidence. Name states/transitions; test adjacency, delimiter transitions, suffix/metadata preservation, termination, escaping/quoting when supported, and EOF/error behavior. Each changed delimiter/state needs adjacency before/after non-whitespace token chars plus delimiter-like text inside protected states when supported; separate adjacent segment means no neighbor merge. For one component inside a structured value, split/compare/recombine it without changing adjacent metadata. Docs/plans tie validation to evidence and responsibility/ownership maps." : undefined,
    ...
    "- Hard budget stop: checkpoint partial handoff, uncertainty, next split/continue. Soft budget warning: finish current evidence thread and stop cleanly.",
    "- Long work: checkpoint, return a handoff, and let the orchestrator continue.",
    ...
    "- Use chalin_artifact_write only at meaningful boundaries: feature-state, checkpoint, validation-contract, or reusable worker-skill.",
...
export function childToolNames(agent: AgentDefinition | undefined, _task = "", needsArtifacts = false, hasPrevious = false, options: ChildToolOptions = {}): string[] {
  ...
  if (needsArtifacts && hasAnyCapability(agent, ["memory-write", "coordinate", "validate", "edit-files"])) names.add("chalin_artifact_write");
