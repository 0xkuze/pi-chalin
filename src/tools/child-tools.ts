...
function createChalinBashJobTool(policy: ChildToolPolicy): ToolDefinition {
  return defineTool<typeof ChalinBashJobParams, unknown>({
    ...
    promptSnippet: "chalin_bash_job: use only for long-timeout, likely slow verification/build/typecheck/full-suite, dev-server, watcher, sleep/server, or independently useful async commands; use bash when you need the result before responding.",
    promptGuidelines: [
      "Use bash by default for ordinary commands, quick checks, and commands whose output you need before the next assistant response.",
      "Use action=start only for commands expected to take long enough that continuing independent work, avoiding session blockage, or avoiding a long timeout is valuable.",
      "Choose this over bash for likely-long full test/build/typecheck/CI/dev-server/watch commands, not for every verification command.",
      "Do not start a background job when the next thing you will do is simply wait for it; use bash with an appropriate timeout instead.",
