...
export function registerChalinTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptSnippet: "chalin_bash_job: use only for long-timeout, likely slow verification/build/typecheck/full-suite, dev-server, watcher, sleep/server, or independently useful async commands; use normal bash when you need the result before responding.",
    promptGuidelines: [
      "Use normal bash by default for ordinary commands, quick checks, and commands whose output you need before the next assistant response.",
      "Use action=start only for commands expected to take long enough that avoiding session blockage, avoiding a long timeout, or continuing independent work is valuable.",
      "Choose this over inline bash for likely-long full test/build/typecheck/CI/dev-server/watch commands, not for every verification command.",
      "Do not start a background job when the next thing you will do is simply wait for it; use normal bash with an appropriate timeout instead.",
