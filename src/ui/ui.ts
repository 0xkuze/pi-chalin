...
function formatActivity(run: RunState): string[] {
  ...
  return [
    ...
    run.workUnits?.length ? `work units: ${run.workUnits.length} · skipped: ${run.steps.filter((step) => step.status === "skipped").length}` : undefined,
    run.recoveryState?.failedStepId ? `recovery: failed ${run.recoveryState.failedStepId} · repair: ${run.recoveryState.repairOptions.join("; ") || "none"}` : undefined,
    ...run.steps.map((step) => `${statusIcon(step.status)} ${step.agent.padEnd(15)} ${formatStepDuration(step)} ${truncateUi(step.skipReason || step.output?.handoff || step.error || step.task, 120)}`),
...
export function formatRunSummary(run: RunState | undefined): string {
  if (!run) return "No pi-chalin run found.";
  return formatActivity(run).filter((line) => !line.startsWith("keys:")).join("\n");
}

export function formatRunInspection(run: RunState | undefined, options: { stepId?: string } = {}): string {
  if (!run) return "No pi-chalin run found.";
  const step = options.stepId ? run.steps.find((candidate) => candidate.id === options.stepId || candidate.agent === options.stepId) : undefined;
  if (step) return formatActivityStep(step);
  const recovery = run.recoveryState ? [
    `failedStep: ${run.recoveryState.failedStepId ?? "none"}`,
    `failedUnit: ${run.recoveryState.failedUnitId ?? "none"}`,
    `reviewersNotRun: ${run.recoveryState.reviewersNotRun.join(", ") || "none"}`,
    `repairOptions: ${run.recoveryState.repairOptions.join("; ") || "none"}`,
  ].join("\n") : "recovery: none";
  const units = (run.workUnits ?? []).slice(0, 12).map((unit) => `- ${unit.id} · ${unit.status} · ${unit.title}`).join("\n");
  return [
    formatRunSummary(run),
    "",
    recovery,
    run.mutationLedger?.length ? `mutationLedger: ${run.mutationLedger.length}` : "mutationLedger: 0",
    run.verificationLedger?.length ? `verificationLedger: ${run.verificationLedger.length}` : "verificationLedger: 0",
    units ? `workUnits:\n${units}` : "workUnits: none",
  ].join("\n");
}
...
function formatActivityStep(step: RunState["steps"][number] | undefined): string {
  ...
  return [
    ...
    step.workUnitId ? `workUnit: ${step.workUnitId}` : undefined,
    step.task,
    step.skipReason ? `skipped: ${step.skipReason}` : undefined,
    step.output?.handoff,
