...
test("resolveVariantsToRun supports single-variant diagnosis", () => {
  assert.deepEqual(resolveVariantsToRun(undefined), ["simple", "chalin"]);
  assert.deepEqual(resolveVariantsToRun("both"), ["simple", "chalin"]);
...
test("extractChalinToolResultText reads completed chalin_route material directly", () => {
  const stdout = [
    JSON.stringify({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: "Final answer material: apps/web services/api packages/ui pnpm test" }] } }),
  ...
  assert.match(extractChalinToolResultText(stdout), /apps\/web services\/api/);
});
