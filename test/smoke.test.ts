...
test("pi-chalin keeps bounded path prompts compact without hiding route autonomy", async () => {
  ...
  assert.match(promptResult?.message?.content ?? "", /Mandatory docs mutation/i);
  assert.match(promptResult?.message?.content ?? "", /Operational docs deadline/i);
  assert.match(promptResult?.message?.content ?? "", /next tool must write or edit the requested docs artifact/i);
  assert.match(promptResult?.message?.content ?? "", /do not answer with a route label/i);
  assert.match(promptResult?.message?.content ?? "", /Start native/i);
  ...
  assert.match(promptResult?.message?.content ?? "", /Direct source candidate from prompt operation `sync`/i);
  assert.match(promptResult?.message?.content ?? "", /read `src\/sync\.ts` before any find/i);
  assert.match(promptResult?.message?.content ?? "", /Use fallback `src\/sync\.js` only if that direct read fails/i);
  assert.match(promptResult?.message?.content ?? "", /Do not read every extension variant after a hit/i);
  assert.match(promptResult?.message?.content ?? "", /do not glob source\/test trees/i);
  assert.match(promptResult?.message?.content ?? "", /do not run test globs before the package runner/i);
  ...
  assert.match(promptResult?.message?.content ?? "", /src\/<name>\.ts/i);
  assert.match(promptResult?.message?.content ?? "", /Do not use find\/grep only to prove tests are absent/i);
  assert.match(promptResult?.message?.content ?? "", /package\.json\/source evidence is enough to document how to run tests/i);
  ...
  assert.match(promptResult?.message?.content ?? "", /final visible block that is a table\/list\/code fence/i);
  assert.match(promptResult?.message?.content ?? "", /End with one complete prose sentence/i);
  ...
  assert.match(promptResult?.message?.content ?? "", /Operational docs compact mode/i);
  assert.match(promptResult?.message?.content ?? "", /make the artifact operational, not exhaustive/i);
  assert.match(promptResult?.message?.content ?? "", /Required headings: `Estado actual`\/`Current state`, `Pasos`\/`Steps`, rollback, quick reference/i);
  assert.match(promptResult?.message?.content ?? "", /Do not run shell before writing unless the user explicitly asks/i);
  assert.match(promptResult?.message?.content ?? "", /run at most one pre-write shell total only when the user explicitly asks/i);
  assert.match(promptResult?.message?.content ?? "", /After evidence, the next mutation must be the docs write/i);
  assert.match(promptResult?.message?.content ?? "", /artifact text itself must name the manifest path/i);
  assert.match(promptResult?.message?.content ?? "", /such as `package\.json`, `Cargo\.toml`, or `pyproject\.toml`/i);
  assert.match(promptResult?.message?.content ?? "", /VCS is absent or not evidenced/i);
  assert.match(promptResult?.message?.content ?? "", /do not present `git restore`\/`git stash`\/`git revert` as runnable current-repo steps/i);
  assert.match(promptResult?.message?.content ?? "", /manual backup\/restore/i);
  assert.match(promptResult?.message?.content ?? "", /application-level retry\/re-run only for pure\/no-side-effect source behavior/i);
  assert.match(promptResult?.message?.content ?? "", /ambiguous no-tests vs failed-assertion output/i);
  assert.match(promptResult?.message?.content ?? "", /Rollback targets the named operation\/system\/change/i);
  assert.match(promptResult?.message?.content ?? "", /compact cases\/invariants only from source facts/i);
  assert.match(promptResult?.message?.content ?? "", /One write, one readback, at most one corrective edit\+readback/i);
  assert.match(promptResult?.message?.content ?? "", /do not iterate on polish/i);
