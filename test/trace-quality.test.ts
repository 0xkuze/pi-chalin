...
test("parsePiJsonTrace extracts chalin_route args and result material", () => {
  const stdout = [
    event({ type: "tool_execution_start", toolName: "chalin_route", args: { prompt: "use chalin_project_discovery first" } }),
...
test("gradePiTrace passes evidence-rich chalin trace", () => {
  const stdout = [
    event({ type: "tool_execution_start", toolName: "chalin_route", args: { prompt: "Call chalin_project_discovery before claims" } }),
    event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: deepAnswer }] } }),
    event({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: "Resumen parcial" }] } }),
  ].join("\n");

  const report = gradePiTrace(stdout, { variant: "chalin", finalText: "Resumen parcial" });
  assert.equal(report.pass, true);
  assert.equal(report.effectiveAnswerSource, "chalin_route");
  assert.equal(report.critical.length, 0);
  assert.ok(report.suggestions.some((issue) => issue.id === "partial-parent-final"));
});

test("gradePiTrace fails chalin traces without chalin_route result", () => {
  ...
  assert.ok(report.critical.some((issue) => issue.id === "missing-chalin-route-result"));
});

test("gradePiTrace flags repeated chalin_route and parent exploration after chalin", () => {
  const stdout = [
    event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: deepAnswer }] } }),
    event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: deepAnswer }] } }),
  ...
  const report = gradePiTrace(stdout, { variant: "chalin" });
  assert.equal(report.pass, false);
  assert.ok(report.warnings.some((issue) => issue.id === "repeated-chalin-route"));
  assert.ok(report.warnings.some((issue) => issue.id === "parent-tools-after-chalin"));
});

test("gradePiTrace treats tools after approval-blocked chalin_route as recovery", () => {
  ...
  const stdout = [
    event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: blockedChalin }] } }),
  ...
  const report = gradePiTrace(stdout, { variant: "chalin", promptKind: "generic", finalText: "Cambios en src/pricing.ts. Verificación: bun test passed.", minAnswerChars: 20 });

  assert.equal(report.metrics.chalinRouteApprovalBlocked, 1);
  assert.equal(report.metrics.exploratoryParentToolsAfterChalin, 0);
...
test("gradePiTrace fails unauditable deep answers without file evidence", () => {
  const stdout = event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: "Este proyecto es importante y tiene buena arquitectura. ".repeat(30) }] } });
...
test("buildTraceJudgePrompt includes deterministic report and strict JSON contract", () => {
  const report = gradePiTrace(event({ type: "tool_execution_end", toolName: "chalin_route", result: { content: [{ type: "text", text: deepAnswer }] } }), { variant: "chalin" });
